...
import styles from './SharePointPeoplePicker.module.scss';
...
export interface ISharePointPeoplePickerProps {
	...
	onItemsChange?: (items: IPersonaProps[]) => void;
}

export interface ISharePointPeoplePickerState {
...
export default class SharePointPeoplePicker extends React.Component<
	ISharePointPeoplePickerProps,
	ISharePointPeoplePickerState
> {
	private _peopleList;
	...
	public render(): React.ReactElement<ISharePointPeoplePickerProps> {
		if (this.props.typePicker == 'Normal') {
			return (
				<NormalPeoplePicker
					...
					itemLimit={this.props.maxSelectedItems}
	...
	@autobind
	private _onItemsChange(items: IPersonaProps[]) {
		console.log('ON ITEMS CHANGE');
   ...
   */
	private _searchPeople(terms: string, results: IPersonaProps[]): IPersonaProps[] | Promise<IPersonaProps[]> {
		...
		if (this.props.siteUrl.toLowerCase().indexOf('wwww.contoso.com') >= 0) {
			...
			const userRequestUrl: string = `${this.props
			...
			return new Promise<IPersonaProps[]>((resolve, reject) =>
              ...
              // Return a Persona element for each suggestion
              let people = relevantResults.map(p => {
                let account = p.Key;
                let accountName = p.Key.substr(p.Key.lastIndexOf('|') + 1);

                return {
                  itemID: accountName,
                  primaryText: p.DisplayText,
                  imageUrl: `/_layouts/15/userphoto.aspx?size=S&accountname=${accountName}`,
